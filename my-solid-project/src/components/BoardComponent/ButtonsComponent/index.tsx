...
const ButtonsComponent: Component<ButtonsProps> = (props: ButtonsProps) => {
    ...
    function handleOnClickAdd(event: MouseEvent) {
        event.stopPropagation();
        setIsOpen(true);
    }

    function handleOnClickAddNode(event: MouseEvent) {
        event.stopPropagation();
    ...
    function handleChangeNumberInputs(event: Event) {
        const target = event.target as HTMLInputElement; // Type assertion
        setNumberInputs(Number(target.value)); // Convert to number
    }

    function handleChangeNumberOutputs(event: Event) {
        const target = event.target as HTMLInputElement; // Type assertion
        setNumberOutputs(Number(target.value)); // Corrected to set numberOutputs
    }

    return (
        <div class={styles.wrapper}>
            <button 
                class={props.showDelete ? styles.buttonDelete : styles.buttonDeleteHidden} 
                onClick={props.onClickDelete}
            >
                <svg
                    fill="currentColor"
                    stroke-width="0" // Changed to strokeWidth
            ...
            <button class={styles.buttonAdd} onClick={handleOnClickAdd}>
                <svg
                    ...
                    style="overflow: visible;"
                >
                    <path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32v144H48c-17.7 0-32 14.3-32 32s14.3 32 32 32h144v144c0 17.7 14.3 32 32 32s32-14.3 32-32V288h144c17.7 0 32-14.3 32-32s14.3-32 -32 -32H256V80z"></path>
                </svg>
            </button>

            <div
                class={isOpen() ? styles.dropdown : styles.dropdownHidden}
                ref={(el) => clickOutside(el, () => setIsOpen(false))} // Use clickOutside function
            >
